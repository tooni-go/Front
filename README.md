...
Entrada de datos simplificada: El docente copia el texto completo de su examen y clave de respuestas desde su procesador de texto habitual y lo pega en un área de texto único (<textarea>).

Estructuración por IA: NestJS envía este texto crudo a Gemini con un System Prompt estricto de extracción.

Generación del formulario: La IA procesa el contenido y devuelve un esquema JSON estructurado con las preguntas separadas, sus respuestas esperadas y una distribución equitativa del puntaje (que suma 100).

Validación docente: El frontend renderiza automáticamente la rúbrica en un formulario dinámico, permitiendo al profesor hacer ajustes menores antes de guardar.
...
Flujo de Trabajo
El profesor inicia sesión con Google.

Crea un examen utilizando el Copy-Paste Inteligente (o formulario manual).

La IA parsea el examen, extrae las preguntas y distribuye el puntaje automáticamente.

Los alumnos realizan el examen en papel.

El profesor crea una nueva entrega y carga imágenes o un PDF.

El backend procesa el archivo y lo envía al proveedor de IA con sus System Prompts y guardrails de seguridad.

El proveedor devuelve una respuesta estructurada en formato JSON con:
...
El backend valida el JSON recibido.

Si el nivel de confianza es bajo, la letra es ilegible o hay preguntas gráficas marked para revisión, la entrega queda en estado REQUIERE_REVISION. Caso contrario, pasa a PENDIENTE_APROBACION.

El profesor revisa la propuesta.

Puede modificar puntajes y completar las preguntas marcadas para revisión.

Aprueba la corrección definitiva (PUBLICADO).
...
Gestión de exámenes (Carga Inteligente & Formulario Dinámico).

Formularios.

Pantallas de corrección y revisión visual.
...
IA e Integración
Prompts de extracción (Copy-Paste a Rúbrica JSON).

Prompts de corrección con Guardrails y Autoevaluación.

Comunicación con Gemini y OpenRouter.
